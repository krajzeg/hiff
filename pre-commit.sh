...
# Check for commits to forbidden directories
if [ -n "$(git diff HEAD --name-only | grep public/)" ]; then
  echo -e "$RED You have files to be commited in public/ - put them in front/*/assets instead.$NORMAL"
  exit 1
fi
